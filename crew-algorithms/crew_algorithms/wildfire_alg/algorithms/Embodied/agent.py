...
from crew_algorithms.wildfire_alg.algorithms.Embodied.__main__ import Config
...
class Agent:
    ...
    def add_message(self, chat: str, source: str, content: str, time: int):

        self.chat_history[chat].append((source, content, time))

    def clean_messages(self, time, max_life):
        for chat in self.chat_history.values():

            removed_message = []
            for message in chat:
                if int(message[2])<(time-max_life):
                    removed_message.append(message)
            for remove in removed_message:
                chat.remove(remove)
    ...
    def generate_perception(self, cfg: Config, agent_states: Dict[int, Tuple[int, int]], global_data:dict):
        ...
        try:
            if self.cfg.envs.llm_model == "gpt":
                client = OpenAI(
                    api_key=self.api_key,
                    timeout=3600,
                )
                response = guarded_chat_completion(client, 
                    model=self.cfg.llms.small_model,
                    messages=[
                ...
                client = OpenAI(
                    api_key=self.api_key,
                    base_url=self.cfg.envs.llm_url,
                    timeout=3600,
                )
                response = guarded_chat_completion(client, 
                    model=self.cfg.llms.small_model,
            ...
            print(f"BadRequestError for Agent_{self.id}: {e}")
            global_data["api_calls"]+=1
            perception = "I am having trouble understanding my surroundings right now."
